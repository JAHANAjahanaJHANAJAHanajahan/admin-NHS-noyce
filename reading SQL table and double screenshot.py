...
class App:
    def __init__(self):
        ...
        self.last_patient_name = None     # For checking if a new patient is present
        self.database_comment = None      # Derived from age (Blue if age >=65, Green otherwise)
        ...
        # Move debug buttons to bottom left so they don't overlap other controls
    ...
    def run(self):
        while self.running:
            self.handle_events()
            self.draw()
            # Update Tkinter output window if it's open
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                # Debug toggle and buttons (now at bottom left)
                ...
                # Toggle OCR on/off when button is clicked (only if both areas are selected)
    ...
    def ocr_loop(self):
        while self.ocr_active:
            ...
            # Process OCR for name area (using same image processing for clarity)
            if self.name_rect:
    ...
    def update_output_window(self):
        ...
        if self.latest_patient_name is not None and self.latest_patient_name != self.last_patient_name:
            self.last_patient_name = self.latest_patient_name
            insert_query = '''
    ...
    def draw(self):
        ...
        if self.debug_mode:
            ...
            self.draw_button(self.debug_drop_table, "Drop Table", (80, 80, 80), (110, 110, 110))
        pygame.display.flip()
