...
class App:
    ...
    def __init__(self):
        ...
        self.debug_drop_table = pygame.Rect(200, 5, 60, 30)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if self.debug_mode:
                    ...
                    if self.debug_drop_table.collidepoint(mouse_pos):
                        # Drop database table if pressed and creates a fresh one
    ...
    def ocr_loop(self):
        while self.ocr_active:
            if self.selected_rect:
                ...
                upscale_factor = 3
                processed = processed.resize((width * upscale_factor, height * upscale_factor),
    ...
    def update_output_window(self):
        ...
        if self.temp_number != self.latest_ocr_number and self.latest_ocr_number is not None:
            self.temp_number = self.latest_ocr_number
            insert_query = '''
            INSERT INTO patient (age, vaccine_type)
            VALUES (?, ?)
            '''
            cursor.execute(insert_query, (self.latest_ocr_number, self.database_comment))
            test.commit()
    ...
    def draw(self):
        ...
        if self.ocr_active:
            ...
            self.draw_button(self.ocr_button_rect, "Begin Screen Reading", (70, 200, 70), (100, 230, 100),
                             disabled=(self.selected_image is None))
