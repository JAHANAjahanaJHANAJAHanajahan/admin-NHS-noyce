...
# Create tables if they don't exist; added data_hash column to patient table for record integrity
cursor.execute('''
...
# Helper function to compute a SHA-256 hash for a record
def compute_record_hash(age, patient_name, vaccine_type):
    # Concatenate the key patient data to form a unique string
    record_str = f"{age}-{patient_name}-{vaccine_type}"
    # Return the SHA-256 hash of the record string
...
class BaseOCRProcessor:
    def process_image(self, screenshot):
        # Common image processing: convert to grayscale, upscale, and threshold
...
# Derived class for processing age (numeric OCR)
class AgeOCRProcessor(BaseOCRProcessor):
    def process_ocr(self, screenshot):
        processed_image = self.process_image(screenshot)
        # Use specific config to whitelist digits
        custom_config = "--psm 7 -c tessedit_char_whitelist=0123456789"
        ocr_result = pytesseract.image_to_string(processed_image, config=custom_config).strip()
        try:
            return int(ocr_result)
        except ValueError:
            return None

# Derived class for processing name (general text OCR)
class NameOCRProcessor(BaseOCRProcessor):
    def process_ocr(self, screenshot):
        processed_image = self.process_image(screenshot)
        # Use default OCR configuration
...
class App:
    ...
    def start_age_selection(self):
        ...
        if result is not None:
            ...
            # Update preview with the raw post-processed screenshot (as before)
    ...
    def ocr_loop(self):
        while self.ocr_active:
            if self.age_rect and self.name_rect:
                ...
                # Convert processed image to RGB for preview update (as in original code)
                ...
                age = self.age_processor.process_ocr(screenshot_age)
                # Process name region
                ...
                name = self.name_processor.process_ocr(screenshot_name)
                # Update OCR results only if both are valid
    ...
    def update_output_window(self):
        ...
        if self.latest_patient_name and self.latest_ocr_number is not None and computed_vaccine:
            # Compute current hash for the record
            ...
            if self.last_processed.get(self.latest_patient_name) != current_data:
                ...
                if row:
                    ...
                    # If the age or hash has changed, update the record
